...
export class SharedService {
  ...
  //recibe como parametro el arreglo con los datos del reporte
  public generarExcelReporteVentasCotizaciones(datos) {
    const title = 'Reporte Ventas Cotizaciones';
    const header = ["ID Cliente", "Cliente", "RFC"]
    const header1 = ["Folio", "Fecha de Expedicion", "Fecha Vigencia", "Total MXN", "Total DLLS", "Flete", "Moneda", "T.C."]
    const data = datos;
    let workbook = new Workbook();
    let worksheet = workbook.addWorksheet('Cotizaciones');
    let titleRow = worksheet.addRow([title]);
    titleRow.font = { name: 'Comic Sans MS', family: 4, size: 16, underline: 'double', bold: true };
    worksheet.addRow([]);
    worksheet.addRow([]);
    ...
    data.forEach((d) => {
      console.log(d);
      let registro = [d.IdClientes, d.Nombre, d.RFC];
      console.log(registro);
      let row = worksheet.addRow(registro);
      ...
      d.Docs.forEach((docs) => {
        ...
        let fechaela = docs.FechaDeExpedicion.substring(0, 10);
        let fechaentre = docs.Vigencia.substring(0, 10);

        let registro2 = [docs.Folio, fechaela, fechaentre, +docs.Total, +docs.TotalDlls, docs.Flete, docs.Moneda, +docs.TipoDeCambio]
        let row2 = worksheet.addRow(registro2);
        totalMXN = totalMXN + +docs.Total;
        totalUSD = totalUSD + +docs.TotalDlls;
...
//recibe como parametro el arreglo con los datos del reporte
public generarExcelReporteVentasPedidos(datos) {
  const title = 'Reporte Ventas Pedidos';
  const header = ["ID Cliente", "Cliente", "RFC"]
  const header1 = ["Folio", "Fecha de Expedicion", "Fecha Vencimineto", "Fecha Entrega", "Total MXN", "Total DLLS", "Flete", "Moneda"]
  const data = datos;
  let workbook = new Workbook();
  ...
  data.forEach((d) => {
    ...
    d.Docs.forEach((docs) => {
      ...
      let fechaExp= docs.FechaDeExpedicion.substring(0, 10);
      let fechaVenci = docs.FechaVencimiento.substring(0, 10);
      let fechaentre = docs.FechaDeEntrega.substring(0, 10);

      let registro2 = [docs.Folio, fechaExp, fechaVenci, fechaentre, +docs.Total, +docs.TotalDlls, docs.Flete, docs.Moneda]
      let row2 = worksheet.addRow(registro2);
      totalMXN = totalMXN + +docs.Total;
      totalUSD = totalUSD + +docs.TotalDlls;
  ...
  workbook.xlsx.writeBuffer().then((data) => {
    let blob = new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    fs.saveAs(blob, 'ReporteVentasPedido.xlsx');
  });
}
