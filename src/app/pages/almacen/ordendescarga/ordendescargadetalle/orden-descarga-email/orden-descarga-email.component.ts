...
@Component({
  selector: 'app-orden-descarga-email',
  templateUrl: './orden-descarga-email.component.html',
  styleUrls: ['./orden-descarga-email.component.css']
})
export class OrdenDescargaEmailComponent implements OnInit {

  constructor(public router: Router, public AlmacenEmailService: AlmacenEmailService, private sanitizer: DomSanitizer, public dialogRef: MatDialogRef<OrdenDescargaEmailComponent>,
    public imageService: ImagenService,  private _sanitizer: DomSanitizer,) { }
...
obtenerDocumentos(){
  ...
  const formData = new FormData();
  formData.append('folio', this.Folio.toString());
  this.AlmacenEmailService.readDirDocuemntosAlmacen(formData, 'cargarNombreDocuemntosOrdenDescarga').subscribe(res=>{
    // console.log(res);
    this.archivos =[];
    if (res){
    ...
    for(let i = 0; i<res.length; i++){
      ...
      this.AlmacenEmailService.readDocumentosAlmacen(formDataDoc,'ObtenerDocumentoOrdenDescarga').subscribe(resDoc=>{
    //  console.log(resDoc)
...
leerArchivos(a){
  ...
  this.AlmacenEmailService.readDocumentosAlmacen(formData,'ObtenerDocumentoOrdenDescarga').subscribe(res=>{
    // console.log(res);
...
onSelect(event){
  ...
  formData.append('folio', this.Folio.toString())
  this.AlmacenEmailService.saveFileAlmacen(formData,'guardarDocumentoOrdenDescarga').subscribe(res=>{
console.log(res);
this.obtenerDocumentos();
  })
}
...
onRemove(event){
  ...
  const formData = new FormData();
      ...
      this.AlmacenEmailService.deleteDocumentoAlmacen(formData,'borrarDocumentoOrdenDescarga').subscribe(res => {
        console.log(res)
