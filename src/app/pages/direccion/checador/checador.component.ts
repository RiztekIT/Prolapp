...
subs2: Subscription
  obtenerInformacionChecador() {
    try {
      ...
      this.userChecador = [];
    this.subs1 =   this.usuarioservice.checadorFechasSemana(fecha1, fecha2).subscribe(dataUsuarios => {
        ...
        this.userChecador.forEach((usuario, key) => {
          ...
          console.log(anio2);
          let diaLimite;
          var date = new Date(fechaInicialSemana);
          var firstDay = new Date(date.getFullYear(), date.getMonth(), 1);
          var lastDay = new Date(date.getFullYear(), date.getMonth() + 1, 0);
          diaLimite = lastDay.toString().split(' ')[2]
          console.log(diaLimite);

         this.subs2 = this.usuarioservice.checadorEarliestDateUsuario(diaLimite,dia, dia2, mes, mes2, anio, anio2, usuario.username).subscribe(data => {
            console.log(data);
            data.forEach((element, index) => {
              ...
              let diaFinal = dia2
              for (let i = 0; i <= 6; i++) {
                // console.log('%c%s', 'color: #00a3cc', contador);
                // console.log('%c%s', 'color: #00e600', i);
                // console.log(i);
                // let semanaInicio = element.informacion.startDate.split('/', 3)
                let day = new Date(element.fecha.format('MM/dd/yyyy')).toString().split(' ')
                // console.log(+day[2]);
                if (+day[2] == diaActual) {
                  // console.log('HACIENDO PUSH');
                  // console.log(element.device.split(':'));
                  let device = element.device.split(':')
                  device = device[1].split('(')
                  device = device[1].split(')')
                  device = device[0];
                  // console.log(device);                
                  element.index = i
                  element.dispositivo = device;
                  this.userChecador[key].checador.push(element)
                }
  ...
  //             // this.applyFilter('');
  //             // console.log(this.userChecador);
  //             console.log('%c⧭', 'color: #ff0000', this.userChecador);
  //           });
  //         });
  //     });
  //   });
